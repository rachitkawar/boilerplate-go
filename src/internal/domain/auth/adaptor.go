...
func (a *AuthSrv) Login() (*models.LoginResponse, error) {
	...
	user, err := a.db.GetUserByEmail(models.LoginRequest{}.Email)
	if err != nil {
	...
	//compare password
	err = bcrypt.CompareHashAndPassword([]byte(user.Password), []byte(models.LoginRequest{}.Password))
	if err != nil {
	...
	//generate token
	token, err := a.generateToken(user, true)
	if err != nil {
...
func (a *AuthSrv) Signup(signupRequest *models.SignupRequest) (*models.SignupResponse, error) {
	...
	//generate token
	token, err := a.generateToken(user, false)
	if err != nil {
		utils.Log.Error("cannot generate token %v", err)
